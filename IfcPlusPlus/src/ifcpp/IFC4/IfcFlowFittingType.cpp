...
#include "include/IfcFlowFittingType.h"
...
shared_ptr<IfcPPObject> IfcFlowFittingType::getDeepCopy( IfcPPCopyOptions& options )
{
...
{
	stream << "#" << m_id << "= IFCFLOWFITTINGTYPE" << "(";
...
void IfcFlowFittingType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFlowFittingType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowFittingType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcFlowFittingType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
