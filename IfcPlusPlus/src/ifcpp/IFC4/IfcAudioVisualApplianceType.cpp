...
#include "include/IfcAudioVisualApplianceType.h"
...
shared_ptr<IfcPPObject> IfcAudioVisualApplianceType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcAudioVisualApplianceType> copy_self( new IfcAudioVisualApplianceType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcAudioVisualApplianceTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcAudioVisualApplianceType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCAUDIOVISUALAPPLIANCETYPE" << "(";
...
void IfcAudioVisualApplianceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcAudioVisualApplianceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAudioVisualApplianceType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAudioVisualApplianceTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcAudioVisualApplianceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
