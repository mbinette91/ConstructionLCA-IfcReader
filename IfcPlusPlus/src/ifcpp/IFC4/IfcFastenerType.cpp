...
IfcFastenerType::IfcFastenerType() { m_entity_enum = IFCFASTENERTYPE; }
IfcFastenerType::IfcFastenerType( int id ) { m_id = id; m_entity_enum = IFCFASTENERTYPE; }
IfcFastenerType::~IfcFastenerType() {}
shared_ptr<IfcPPObject> IfcFastenerType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcFastenerType> copy_self( new IfcFastenerType() );
	if( m_GlobalId )
	{
...
void IfcFastenerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFastenerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcFastenerTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcFastenerType::unlinkFromInverseCounterparts()
{
