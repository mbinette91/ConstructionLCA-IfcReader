...
#include "include/IfcFlowTreatmentDeviceType.h"
...
IfcFlowTreatmentDeviceType::IfcFlowTreatmentDeviceType() { m_entity_enum = IFCFLOWTREATMENTDEVICETYPE; }
IfcFlowTreatmentDeviceType::IfcFlowTreatmentDeviceType( int id ) { m_id = id; m_entity_enum = IFCFLOWTREATMENTDEVICETYPE; }
IfcFlowTreatmentDeviceType::~IfcFlowTreatmentDeviceType() {}
shared_ptr<IfcPPObject> IfcFlowTreatmentDeviceType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcFlowTreatmentDeviceType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCFLOWTREATMENTDEVICETYPE" << "(";
...
void IfcFlowTreatmentDeviceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFlowTreatmentDeviceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowTreatmentDeviceType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcFlowTreatmentDeviceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcFlowTreatmentDeviceType::unlinkFromInverseCounterparts()
{
