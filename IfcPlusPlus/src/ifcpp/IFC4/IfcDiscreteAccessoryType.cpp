...
#include "include/IfcDiscreteAccessoryType.h"
...
shared_ptr<IfcPPObject> IfcDiscreteAccessoryType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDiscreteAccessoryType> copy_self( new IfcDiscreteAccessoryType() );
	if( m_GlobalId )
	{
...
void IfcDiscreteAccessoryType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDISCRETEACCESSORYTYPE" << "(";
...
void IfcDiscreteAccessoryType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcDiscreteAccessoryType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDiscreteAccessoryType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDiscreteAccessoryTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcDiscreteAccessoryType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcDiscreteAccessoryType::unlinkFromInverseCounterparts()
{
