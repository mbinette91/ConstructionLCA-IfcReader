...
#include "include/IfcBuildingElementProxyType.h"
...
IfcBuildingElementProxyType::IfcBuildingElementProxyType() { m_entity_enum = IFCBUILDINGELEMENTPROXYTYPE; }
IfcBuildingElementProxyType::IfcBuildingElementProxyType( int id ) { m_id = id; m_entity_enum = IFCBUILDINGELEMENTPROXYTYPE; }
IfcBuildingElementProxyType::~IfcBuildingElementProxyType() {}
shared_ptr<IfcPPObject> IfcBuildingElementProxyType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcBuildingElementProxyType> copy_self( new IfcBuildingElementProxyType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcBuildingElementProxyTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcBuildingElementProxyType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCBUILDINGELEMENTPROXYTYPE" << "(";
...
void IfcBuildingElementProxyType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBuildingElementProxyType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBuildingElementProxyTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcBuildingElementProxyType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcBuildingElementProxyType::unlinkFromInverseCounterparts()
{
