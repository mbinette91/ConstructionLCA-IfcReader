...
#include "include/IfcColumnType.h"
...
shared_ptr<IfcPPObject> IfcColumnType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcColumnType> copy_self( new IfcColumnType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcColumnTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcColumnType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOLUMNTYPE" << "(";
...
void IfcColumnType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcColumnType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcColumnType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcColumnTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcColumnType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcColumnType::unlinkFromInverseCounterparts()
{
