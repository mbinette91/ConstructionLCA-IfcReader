...
#include "include/IfcControllerType.h"
...
shared_ptr<IfcPPObject> IfcControllerType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcControllerType> copy_self( new IfcControllerType() );
	if( m_GlobalId )
	{
...
void IfcControllerType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCONTROLLERTYPE" << "(";
...
void IfcControllerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcControllerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcControllerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcControllerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcControllerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcDistributionControlElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcControllerType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcDistributionControlElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcControllerType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcDistributionControlElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcControllerType::unlinkFromInverseCounterparts()
{
