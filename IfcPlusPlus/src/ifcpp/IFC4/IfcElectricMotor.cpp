...
#include "include/IfcElectricMotor.h"
...
IfcElectricMotor::IfcElectricMotor() { m_entity_enum = IFCELECTRICMOTOR; }
IfcElectricMotor::IfcElectricMotor( int id ) { m_id = id; m_entity_enum = IFCELECTRICMOTOR; }
IfcElectricMotor::~IfcElectricMotor() {}
shared_ptr<IfcPPObject> IfcElectricMotor::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcElectricMotor> copy_self( new IfcElectricMotor() );
	if( m_GlobalId )
	{
...
void IfcElectricMotor::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELECTRICMOTOR" << "(";
...
void IfcElectricMotor::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcElectricMotor::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElectricMotor, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcElectricMotorTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcElectricMotor::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcElectricMotor::unlinkFromInverseCounterparts()
{
