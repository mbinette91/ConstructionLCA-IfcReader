...
#include "include/IfcJunctionBox.h"
...
shared_ptr<IfcPPObject> IfcJunctionBox::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcJunctionBox> copy_self( new IfcJunctionBox() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcJunctionBoxTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcJunctionBox::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCJUNCTIONBOX" << "(";
...
void IfcJunctionBox::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcJunctionBox, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcJunctionBoxTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcJunctionBox::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowFitting::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcJunctionBox::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowFitting::getAttributesInverse( vec_attributes_inverse );
}
void IfcJunctionBox::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
