...
shared_ptr<IfcPPObject> IfcActionRequest::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcActionRequest::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcActionRequestTypeEnum::createObjectFromSTEP( args[6] );
	m_Status = IfcLabel::createObjectFromSTEP( args[7] );
	m_LongDescription = IfcText::createObjectFromSTEP( args[8] );
}
void IfcActionRequest::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
