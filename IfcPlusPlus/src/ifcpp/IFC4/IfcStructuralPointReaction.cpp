...
#include "include/IfcStructuralPointReaction.h"
#include "include/IfcText.h"

// ENTITY IfcStructuralPointReaction 
IfcStructuralPointReaction::IfcStructuralPointReaction() { m_entity_enum = IFCSTRUCTURALPOINTREACTION; }
IfcStructuralPointReaction::IfcStructuralPointReaction( int id ) { m_id = id; m_entity_enum = IFCSTRUCTURALPOINTREACTION; }
...
{
	stream << "#" << m_id << "= IFCSTRUCTURALPOINTREACTION" << "(";
...
void IfcStructuralPointReaction::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralPointReaction, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcStructuralPointReaction::unlinkFromInverseCounterparts()
{
