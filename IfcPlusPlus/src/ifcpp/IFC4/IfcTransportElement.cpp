...
shared_ptr<IfcPPObject> IfcTransportElement::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcTransportElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTransportElement, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcTransportElementTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcTransportElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
