...
#include "include/IfcVoidingFeature.h"
...
shared_ptr<IfcPPObject> IfcVoidingFeature::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcVoidingFeatureTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcVoidingFeature::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCVOIDINGFEATURE" << "(";
...
void IfcVoidingFeature::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcVoidingFeature, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcVoidingFeatureTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcVoidingFeature::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
