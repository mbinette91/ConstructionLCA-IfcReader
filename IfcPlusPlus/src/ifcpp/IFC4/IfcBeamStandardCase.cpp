...
#include "include/IfcBeamStandardCase.h"
...
void IfcBeamStandardCase::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCBEAMSTANDARDCASE" << "(";
...
void IfcBeamStandardCase::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcBeamStandardCase::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
...
void IfcBeamStandardCase::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcBeam::getAttributes( vec_attributes );
}
