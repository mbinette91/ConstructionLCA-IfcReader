...
#include "include/IfcTubeBundleType.h"
...
shared_ptr<IfcPPObject> IfcTubeBundleType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcTubeBundleType> copy_self( new IfcTubeBundleType() );
	if( m_GlobalId )
	{
...
void IfcTubeBundleType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCTUBEBUNDLETYPE" << "(";
...
void IfcTubeBundleType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcTubeBundleType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTubeBundleType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcTubeBundleTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcTubeBundleType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
