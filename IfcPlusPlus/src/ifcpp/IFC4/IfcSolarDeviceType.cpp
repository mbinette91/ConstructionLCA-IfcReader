...
#include "include/IfcSolarDeviceType.h"
...
shared_ptr<IfcPPObject> IfcSolarDeviceType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcSolarDeviceType> copy_self( new IfcSolarDeviceType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcSolarDeviceTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcSolarDeviceType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSOLARDEVICETYPE" << "(";
...
void IfcSolarDeviceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSolarDeviceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSolarDeviceType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSolarDeviceTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcSolarDeviceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcSolarDeviceType::unlinkFromInverseCounterparts()
{
