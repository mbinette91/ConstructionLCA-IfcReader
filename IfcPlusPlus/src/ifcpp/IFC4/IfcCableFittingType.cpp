...
#include "include/IfcCableFittingType.h"
...
IfcCableFittingType::IfcCableFittingType() { m_entity_enum = IFCCABLEFITTINGTYPE; }
IfcCableFittingType::IfcCableFittingType( int id ) { m_id = id; m_entity_enum = IFCCABLEFITTINGTYPE; }
IfcCableFittingType::~IfcCableFittingType() {}
shared_ptr<IfcPPObject> IfcCableFittingType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCableFittingType> copy_self( new IfcCableFittingType() );
	if( m_GlobalId )
	{
...
void IfcCableFittingType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCABLEFITTINGTYPE" << "(";
...
void IfcCableFittingType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCableFittingType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCableFittingTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcCableFittingType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowFittingType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcCableFittingType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowFittingType::getAttributesInverse( vec_attributes_inverse );
}
void IfcCableFittingType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
