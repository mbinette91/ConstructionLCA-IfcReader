...
#include "include/IfcRampFlight.h"
...
IfcRampFlight::IfcRampFlight() { m_entity_enum = IFCRAMPFLIGHT; }
IfcRampFlight::IfcRampFlight( int id ) { m_id = id; m_entity_enum = IFCRAMPFLIGHT; }
IfcRampFlight::~IfcRampFlight() {}
shared_ptr<IfcPPObject> IfcRampFlight::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcRampFlight> copy_self( new IfcRampFlight() );
	if( m_GlobalId )
	{
...
void IfcRampFlight::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCRAMPFLIGHT" << "(";
...
void IfcRampFlight::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcRampFlight::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRampFlight, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcRampFlightTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcRampFlight::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
