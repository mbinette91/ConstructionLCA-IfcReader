...
{
	shared_ptr<IfcDoorStandardCase> copy_self( new IfcDoorStandardCase() );
	if( m_GlobalId )
	{
...
void IfcDoorStandardCase::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
