...
#include "include/IfcFlowFitting.h"
...
IfcFlowFitting::IfcFlowFitting() { m_entity_enum = IFCFLOWFITTING; }
IfcFlowFitting::IfcFlowFitting( int id ) { m_id = id; m_entity_enum = IFCFLOWFITTING; }
IfcFlowFitting::~IfcFlowFitting() {}
shared_ptr<IfcPPObject> IfcFlowFitting::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcFlowFitting::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCFLOWFITTING" << "(";
...
void IfcFlowFitting::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowFitting, expecting 8, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcFlowFitting::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcDistributionFlowElement::getAttributes( vec_attributes );
}
void IfcFlowFitting::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcDistributionFlowElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcFlowFitting::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
