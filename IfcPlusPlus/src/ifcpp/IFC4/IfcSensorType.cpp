...
#include "include/IfcSensorType.h"
...
shared_ptr<IfcPPObject> IfcSensorType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcSensorTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcSensorType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSENSORTYPE" << "(";
...
void IfcSensorType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSensorType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSensorType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSensorTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcSensorType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcDistributionControlElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcSensorType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcDistributionControlElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcSensorType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcDistributionControlElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcSensorType::unlinkFromInverseCounterparts()
{
