...
#include "include/IfcHeatExchangerType.h"
...
shared_ptr<IfcPPObject> IfcHeatExchangerType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcHeatExchangerType> copy_self( new IfcHeatExchangerType() );
	if( m_GlobalId )
	{
...
void IfcHeatExchangerType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCHEATEXCHANGERTYPE" << "(";
...
void IfcHeatExchangerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcHeatExchangerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcHeatExchangerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcHeatExchangerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcHeatExchangerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcHeatExchangerType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcHeatExchangerType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcEnergyConversionDeviceType::setInverseCounterparts( ptr_self_entity );
}
void IfcHeatExchangerType::unlinkFromInverseCounterparts()
{
