...
#include "include/IfcTransportElementType.h"
...
shared_ptr<IfcPPObject> IfcTransportElementType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcTransportElementType> copy_self( new IfcTransportElementType() );
	if( m_GlobalId )
	{
...
{
	stream << "#" << m_id << "= IFCTRANSPORTELEMENTTYPE" << "(";
...
void IfcTransportElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcTransportElementTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcTransportElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcTransportElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcTransportElementType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
