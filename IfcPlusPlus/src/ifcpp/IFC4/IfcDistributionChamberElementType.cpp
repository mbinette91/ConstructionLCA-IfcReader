...
#include "include/IfcDistributionChamberElementType.h"
...
IfcDistributionChamberElementType::IfcDistributionChamberElementType() { m_entity_enum = IFCDISTRIBUTIONCHAMBERELEMENTTYPE; }
IfcDistributionChamberElementType::IfcDistributionChamberElementType( int id ) { m_id = id; m_entity_enum = IFCDISTRIBUTIONCHAMBERELEMENTTYPE; }
IfcDistributionChamberElementType::~IfcDistributionChamberElementType() {}
shared_ptr<IfcPPObject> IfcDistributionChamberElementType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDistributionChamberElementType> copy_self( new IfcDistributionChamberElementType() );
	if( m_GlobalId )
	{
...
void IfcDistributionChamberElementType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDISTRIBUTIONCHAMBERELEMENTTYPE" << "(";
...
void IfcDistributionChamberElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDistributionChamberElementType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDistributionChamberElementTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcDistributionChamberElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcDistributionChamberElementType::unlinkFromInverseCounterparts()
{
