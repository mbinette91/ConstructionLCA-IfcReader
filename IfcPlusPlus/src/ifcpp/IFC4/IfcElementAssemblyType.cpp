...
IfcElementAssemblyType::IfcElementAssemblyType() { m_entity_enum = IFCELEMENTASSEMBLYTYPE; }
IfcElementAssemblyType::IfcElementAssemblyType( int id ) { m_id = id; m_entity_enum = IFCELEMENTASSEMBLYTYPE; }
...
void IfcElementAssemblyType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementAssemblyType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
