...
#include "include/IfcElementComponentType.h"
...
IfcElementComponentType::IfcElementComponentType() { m_entity_enum = IFCELEMENTCOMPONENTTYPE; }
IfcElementComponentType::IfcElementComponentType( int id ) { m_id = id; m_entity_enum = IFCELEMENTCOMPONENTTYPE; }
IfcElementComponentType::~IfcElementComponentType() {}
shared_ptr<IfcPPObject> IfcElementComponentType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcElementComponentType> copy_self( new IfcElementComponentType() );
	if( m_GlobalId )
	{
...
void IfcElementComponentType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELEMENTCOMPONENTTYPE" << "(";
...
void IfcElementComponentType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementComponentType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcElementComponentType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcElementType::getAttributes( vec_attributes );
}
void IfcElementComponentType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcElementComponentType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcElementComponentType::unlinkFromInverseCounterparts()
{
