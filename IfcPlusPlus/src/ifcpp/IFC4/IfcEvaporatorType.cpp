...
#include "include/IfcEvaporatorType.h"
...
IfcEvaporatorType::IfcEvaporatorType() { m_entity_enum = IFCEVAPORATORTYPE; }
IfcEvaporatorType::IfcEvaporatorType( int id ) { m_id = id; m_entity_enum = IFCEVAPORATORTYPE; }
IfcEvaporatorType::~IfcEvaporatorType() {}
shared_ptr<IfcPPObject> IfcEvaporatorType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcEvaporatorType> copy_self( new IfcEvaporatorType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcEvaporatorTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
...
void IfcEvaporatorType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcEvaporatorTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcEvaporatorType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcEvaporatorType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcEvaporatorType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcEnergyConversionDeviceType::setInverseCounterparts( ptr_self_entity );
}
void IfcEvaporatorType::unlinkFromInverseCounterparts()
{
