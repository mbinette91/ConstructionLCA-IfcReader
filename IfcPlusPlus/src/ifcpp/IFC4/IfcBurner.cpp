...
IfcBurner::~IfcBurner() {}
shared_ptr<IfcPPObject> IfcBurner::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcBurner::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBurner, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBurnerTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcBurner::unlinkFromInverseCounterparts()
{
