...
#include "include/IfcActuatorType.h"
...
shared_ptr<IfcPPObject> IfcActuatorType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcActuatorType> copy_self( new IfcActuatorType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcActuatorTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcActuatorType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCACTUATORTYPE" << "(";
...
void IfcActuatorType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcActuatorType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcActuatorType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcActuatorTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcActuatorType::unlinkFromInverseCounterparts()
{
