...
#include "include/IfcDuctFittingType.h"
...
shared_ptr<IfcPPObject> IfcDuctFittingType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDuctFittingType> copy_self( new IfcDuctFittingType() );
	if( m_GlobalId )
	{
...
void IfcDuctFittingType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDUCTFITTINGTYPE" << "(";
...
void IfcDuctFittingType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcDuctFittingType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDuctFittingType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDuctFittingTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcDuctFittingType::unlinkFromInverseCounterparts()
{
