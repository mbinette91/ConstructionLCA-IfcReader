...
#include "include/IfcDuctSilencer.h"
...
shared_ptr<IfcPPObject> IfcDuctSilencer::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDuctSilencer> copy_self( new IfcDuctSilencer() );
	if( m_GlobalId )
	{
...
void IfcDuctSilencer::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDUCTSILENCER" << "(";
...
void IfcDuctSilencer::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcDuctSilencerTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcDuctSilencer::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcDuctSilencer::unlinkFromInverseCounterparts()
{
