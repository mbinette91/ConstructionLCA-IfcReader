...
#include "include/IfcPipeSegmentType.h"
...
IfcPipeSegmentType::IfcPipeSegmentType() { m_entity_enum = IFCPIPESEGMENTTYPE; }
IfcPipeSegmentType::IfcPipeSegmentType( int id ) { m_id = id; m_entity_enum = IFCPIPESEGMENTTYPE; }
IfcPipeSegmentType::~IfcPipeSegmentType() {}
shared_ptr<IfcPPObject> IfcPipeSegmentType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcPipeSegmentType> copy_self( new IfcPipeSegmentType() );
	if( m_GlobalId )
	{
...
void IfcPipeSegmentType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPIPESEGMENTTYPE" << "(";
...
void IfcPipeSegmentType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPipeSegmentType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPipeSegmentType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcPipeSegmentTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcPipeSegmentType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowSegmentType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcPipeSegmentType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowSegmentType::getAttributesInverse( vec_attributes_inverse );
}
void IfcPipeSegmentType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
