...
#include "include/IfcElectricDistributionBoard.h"
...
IfcElectricDistributionBoard::IfcElectricDistributionBoard() { m_entity_enum = IFCELECTRICDISTRIBUTIONBOARD; }
IfcElectricDistributionBoard::IfcElectricDistributionBoard( int id ) { m_id = id; m_entity_enum = IFCELECTRICDISTRIBUTIONBOARD; }
IfcElectricDistributionBoard::~IfcElectricDistributionBoard() {}
shared_ptr<IfcPPObject> IfcElectricDistributionBoard::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcElectricDistributionBoard> copy_self( new IfcElectricDistributionBoard() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcElectricDistributionBoardTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcElectricDistributionBoard::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELECTRICDISTRIBUTIONBOARD" << "(";
...
void IfcElectricDistributionBoard::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElectricDistributionBoard, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcElectricDistributionBoardTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcElectricDistributionBoard::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcElectricDistributionBoard::unlinkFromInverseCounterparts()
{
