...
IfcRoof::~IfcRoof() {}
shared_ptr<IfcPPObject> IfcRoof::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcRoof> copy_self( new IfcRoof() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcRoofTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcRoof::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCROOF" << "(";
...
void IfcRoof::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRoof, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcRoofTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcRoof::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcBuildingElement::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcRoof::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcBuildingElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcRoof::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcBuildingElement::setInverseCounterparts( ptr_self_entity );
}
void IfcRoof::unlinkFromInverseCounterparts()
{
