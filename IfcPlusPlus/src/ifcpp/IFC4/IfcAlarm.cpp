...
IfcAlarm::~IfcAlarm() {}
shared_ptr<IfcPPObject> IfcAlarm::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcAlarm> copy_self( new IfcAlarm() );
	if( m_GlobalId )
	{
...
void IfcAlarm::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCALARM" << "(";
...
void IfcAlarm::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAlarm, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAlarmTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcAlarm::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
