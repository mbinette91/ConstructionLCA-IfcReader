...
shared_ptr<IfcPPObject> IfcRailing::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcRailing::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCRAILING" << "(";
...
void IfcRailing::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcRailing::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRailing, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcRailingTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcRailing::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcRailing::unlinkFromInverseCounterparts()
{
