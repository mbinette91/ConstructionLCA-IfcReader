...
#include "include/IfcCompressor.h"
...
IfcCompressor::IfcCompressor() { m_entity_enum = IFCCOMPRESSOR; }
IfcCompressor::IfcCompressor( int id ) { m_id = id; m_entity_enum = IFCCOMPRESSOR; }
IfcCompressor::~IfcCompressor() {}
shared_ptr<IfcPPObject> IfcCompressor::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcCompressorTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcCompressor::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOMPRESSOR" << "(";
...
void IfcCompressor::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCompressor::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCompressor, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCompressorTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCompressor::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCompressor::unlinkFromInverseCounterparts()
{
