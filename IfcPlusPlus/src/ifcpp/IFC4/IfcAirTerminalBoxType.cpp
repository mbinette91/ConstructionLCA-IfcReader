...
#include "include/IfcAirTerminalBoxType.h"
...
IfcAirTerminalBoxType::IfcAirTerminalBoxType() { m_entity_enum = IFCAIRTERMINALBOXTYPE; }
IfcAirTerminalBoxType::IfcAirTerminalBoxType( int id ) { m_id = id; m_entity_enum = IFCAIRTERMINALBOXTYPE; }
...
void IfcAirTerminalBoxType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCAIRTERMINALBOXTYPE" << "(";
...
void IfcAirTerminalBoxType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcAirTerminalBoxType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAirTerminalBoxType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAirTerminalBoxTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcAirTerminalBoxType::unlinkFromInverseCounterparts()
{
