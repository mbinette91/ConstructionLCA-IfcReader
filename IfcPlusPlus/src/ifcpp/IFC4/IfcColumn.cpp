...
// ENTITY IfcColumn 
IfcColumn::IfcColumn() { m_entity_enum = IFCCOLUMN; }
IfcColumn::IfcColumn( int id ) { m_id = id; m_entity_enum = IFCCOLUMN; }
IfcColumn::~IfcColumn() {}
shared_ptr<IfcPPObject> IfcColumn::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcColumn> copy_self( new IfcColumn() );
	if( m_GlobalId )
	{
...
void IfcColumn::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcColumnTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcColumn::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcBuildingElement::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcColumn::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcBuildingElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcColumn::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcBuildingElement::setInverseCounterparts( ptr_self_entity );
}
void IfcColumn::unlinkFromInverseCounterparts()
{
