...
#include "include/IfcPipeFittingType.h"
...
shared_ptr<IfcPPObject> IfcPipeFittingType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcPipeFittingType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPIPEFITTINGTYPE" << "(";
...
void IfcPipeFittingType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPipeFittingType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPipeFittingType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcPipeFittingTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcPipeFittingType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcPipeFittingType::unlinkFromInverseCounterparts()
{
