...
shared_ptr<IfcPPObject> IfcWallType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcWallType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWallType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcWallTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcWallType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcWallType::unlinkFromInverseCounterparts()
{
