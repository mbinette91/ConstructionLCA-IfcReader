...
#include "include/IfcCoolingTower.h"
...
IfcCoolingTower::IfcCoolingTower() { m_entity_enum = IFCCOOLINGTOWER; }
IfcCoolingTower::IfcCoolingTower( int id ) { m_id = id; m_entity_enum = IFCCOOLINGTOWER; }
IfcCoolingTower::~IfcCoolingTower() {}
shared_ptr<IfcPPObject> IfcCoolingTower::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCoolingTower> copy_self( new IfcCoolingTower() );
	if( m_GlobalId )
	{
...
void IfcCoolingTower::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOOLINGTOWER" << "(";
...
void IfcCoolingTower::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCoolingTower, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCoolingTowerTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCoolingTower::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCoolingTower::unlinkFromInverseCounterparts()
{
