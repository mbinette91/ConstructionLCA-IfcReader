...
#include "include/IfcMotorConnection.h"
...
shared_ptr<IfcPPObject> IfcMotorConnection::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcMotorConnection> copy_self( new IfcMotorConnection() );
	if( m_GlobalId )
	{
...
void IfcMotorConnection::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCMOTORCONNECTION" << "(";
...
void IfcMotorConnection::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMotorConnection, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcMotorConnectionTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcMotorConnection::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcMotorConnection::unlinkFromInverseCounterparts()
{
