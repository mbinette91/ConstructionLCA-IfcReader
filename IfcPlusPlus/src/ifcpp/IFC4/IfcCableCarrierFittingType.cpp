...
#include "include/IfcCableCarrierFittingType.h"
...
IfcCableCarrierFittingType::IfcCableCarrierFittingType() { m_entity_enum = IFCCABLECARRIERFITTINGTYPE; }
IfcCableCarrierFittingType::IfcCableCarrierFittingType( int id ) { m_id = id; m_entity_enum = IFCCABLECARRIERFITTINGTYPE; }
IfcCableCarrierFittingType::~IfcCableCarrierFittingType() {}
shared_ptr<IfcPPObject> IfcCableCarrierFittingType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCableCarrierFittingType> copy_self( new IfcCableCarrierFittingType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcCableCarrierFittingTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcCableCarrierFittingType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCABLECARRIERFITTINGTYPE" << "(";
...
void IfcCableCarrierFittingType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCableCarrierFittingType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCableCarrierFittingType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCableCarrierFittingTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcCableCarrierFittingType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCableCarrierFittingType::unlinkFromInverseCounterparts()
{
