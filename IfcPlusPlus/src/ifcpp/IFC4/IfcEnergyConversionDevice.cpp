...
#include "include/IfcEnergyConversionDevice.h"
...
shared_ptr<IfcPPObject> IfcEnergyConversionDevice::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcEnergyConversionDevice> copy_self( new IfcEnergyConversionDevice() );
	if( m_GlobalId )
	{
...
void IfcEnergyConversionDevice::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCENERGYCONVERSIONDEVICE" << "(";
...
void IfcEnergyConversionDevice::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEnergyConversionDevice, expecting 8, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcEnergyConversionDevice::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcEnergyConversionDevice::unlinkFromInverseCounterparts()
{
