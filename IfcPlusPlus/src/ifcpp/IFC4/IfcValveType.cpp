...
#include "include/IfcValveType.h"
#include "include/IfcValveTypeEnum.h"

// ENTITY IfcValveType 
IfcValveType::IfcValveType() { m_entity_enum = IFCVALVETYPE; }
IfcValveType::IfcValveType( int id ) { m_id = id; m_entity_enum = IFCVALVETYPE; }
IfcValveType::~IfcValveType() {}
shared_ptr<IfcPPObject> IfcValveType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcValveTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcValveType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCVALVETYPE" << "(";
...
void IfcValveType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcValveType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcValveType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcValveTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcValveType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowControllerType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcValveType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowControllerType::getAttributesInverse( vec_attributes_inverse );
}
void IfcValveType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
