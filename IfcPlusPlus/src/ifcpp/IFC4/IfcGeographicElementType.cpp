...
#include "include/IfcGeographicElementType.h"
...
void IfcGeographicElementType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCGEOGRAPHICELEMENTTYPE" << "(";
...
void IfcGeographicElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcGeographicElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcGeographicElementTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcGeographicElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcGeographicElementType::unlinkFromInverseCounterparts()
{
