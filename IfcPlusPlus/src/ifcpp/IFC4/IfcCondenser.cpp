...
#include "include/IfcCondenser.h"
...
// ENTITY IfcCondenser 
IfcCondenser::IfcCondenser() { m_entity_enum = IFCCONDENSER; }
IfcCondenser::IfcCondenser( int id ) { m_id = id; m_entity_enum = IFCCONDENSER; }
IfcCondenser::~IfcCondenser() {}
shared_ptr<IfcPPObject> IfcCondenser::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCondenser> copy_self( new IfcCondenser() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcCondenserTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcCondenser::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCONDENSER" << "(";
...
void IfcCondenser::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcCondenserTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCondenser::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDevice::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcCondenser::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDevice::getAttributesInverse( vec_attributes_inverse );
}
void IfcCondenser::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcEnergyConversionDevice::setInverseCounterparts( ptr_self_entity );
}
void IfcCondenser::unlinkFromInverseCounterparts()
{
