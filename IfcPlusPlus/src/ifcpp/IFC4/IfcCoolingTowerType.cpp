...
#include "include/IfcCoolingTowerType.h"
...
{
	stream << "#" << m_id << "= IFCCOOLINGTOWERTYPE" << "(";
...
void IfcCoolingTowerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcCoolingTowerTypeEnum::createObjectFromSTEP( args[9] );
}
