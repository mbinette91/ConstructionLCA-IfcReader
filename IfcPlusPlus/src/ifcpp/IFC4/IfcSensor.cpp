...
IfcSensor::~IfcSensor() {}
shared_ptr<IfcPPObject> IfcSensor::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcSensor> copy_self( new IfcSensor() );
	if( m_GlobalId )
	{
...
void IfcSensor::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSENSOR" << "(";
...
void IfcSensor::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSensor::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSensor, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSensorTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcSensor::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcSensor::unlinkFromInverseCounterparts()
{
