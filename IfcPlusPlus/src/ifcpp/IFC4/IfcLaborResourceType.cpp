...
shared_ptr<IfcPPObject> IfcLaborResourceType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcLaborResourceType> copy_self( new IfcLaborResourceType() );
	if( m_GlobalId )
	{
...
void IfcLaborResourceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 12 ){ std::stringstream err; err << "Wrong parameter count for entity IfcLaborResourceType, expecting 12, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcLaborResourceTypeEnum::createObjectFromSTEP( args[11] );
}
...
void IfcLaborResourceType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcLaborResourceType::unlinkFromInverseCounterparts()
{
