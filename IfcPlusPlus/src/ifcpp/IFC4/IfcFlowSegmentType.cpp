...
#include "include/IfcFlowSegmentType.h"
...
IfcFlowSegmentType::IfcFlowSegmentType() { m_entity_enum = IFCFLOWSEGMENTTYPE; }
IfcFlowSegmentType::IfcFlowSegmentType( int id ) { m_id = id; m_entity_enum = IFCFLOWSEGMENTTYPE; }
IfcFlowSegmentType::~IfcFlowSegmentType() {}
shared_ptr<IfcPPObject> IfcFlowSegmentType::getDeepCopy( IfcPPCopyOptions& options )
{
...
{
	stream << "#" << m_id << "= IFCFLOWSEGMENTTYPE" << "(";
...
void IfcFlowSegmentType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFlowSegmentType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowSegmentType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcFlowSegmentType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
