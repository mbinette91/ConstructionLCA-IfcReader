...
shared_ptr<IfcPPObject> IfcMechanicalFastener::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcMechanicalFastener::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMechanicalFastener, expecting 11, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_NominalDiameter = IfcPositiveLengthMeasure::createObjectFromSTEP( args[8] );
	m_NominalLength = IfcPositiveLengthMeasure::createObjectFromSTEP( args[9] );
	m_PredefinedType = IfcMechanicalFastenerTypeEnum::createObjectFromSTEP( args[10] );
}
...
void IfcMechanicalFastener::unlinkFromInverseCounterparts()
{
