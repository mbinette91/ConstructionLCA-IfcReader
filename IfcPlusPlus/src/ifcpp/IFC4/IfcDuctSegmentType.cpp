...
#include "include/IfcDuctSegmentType.h"
...
shared_ptr<IfcPPObject> IfcDuctSegmentType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcDuctSegmentType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDUCTSEGMENTTYPE" << "(";
...
void IfcDuctSegmentType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcDuctSegmentType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDuctSegmentType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDuctSegmentTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcDuctSegmentType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcDuctSegmentType::unlinkFromInverseCounterparts()
{
