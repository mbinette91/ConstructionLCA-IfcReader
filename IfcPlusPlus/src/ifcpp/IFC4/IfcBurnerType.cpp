...
#include "include/IfcBurnerType.h"
...
IfcBurnerType::IfcBurnerType() { m_entity_enum = IFCBURNERTYPE; }
IfcBurnerType::IfcBurnerType( int id ) { m_id = id; m_entity_enum = IFCBURNERTYPE; }
IfcBurnerType::~IfcBurnerType() {}
shared_ptr<IfcPPObject> IfcBurnerType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcBurnerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcBurnerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBurnerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBurnerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcBurnerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
