...
#include "include/IfcCompressorType.h"
...
shared_ptr<IfcPPObject> IfcCompressorType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCompressorType> copy_self( new IfcCompressorType() );
	if( m_GlobalId )
	{
...
void IfcCompressorType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOMPRESSORTYPE" << "(";
...
void IfcCompressorType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCompressorType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCompressorType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCompressorTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcCompressorType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
