...
shared_ptr<IfcPPObject> IfcChimney::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcChimney::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCHIMNEY" << "(";
...
void IfcChimney::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcChimney::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcChimney, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcChimneyTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcChimney::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
