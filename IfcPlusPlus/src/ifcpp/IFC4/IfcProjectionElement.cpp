...
#include "include/IfcProjectionElement.h"
...
shared_ptr<IfcPPObject> IfcProjectionElement::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcProjectionElementTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcProjectionElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPROJECTIONELEMENT" << "(";
...
void IfcProjectionElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcProjectionElement, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcProjectionElementTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcProjectionElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFeatureElementAddition::getAttributes( vec_attributes );
