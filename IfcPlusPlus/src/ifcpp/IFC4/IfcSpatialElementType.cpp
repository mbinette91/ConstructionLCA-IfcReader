...
#include "include/IfcSpatialElementType.h"
...
shared_ptr<IfcPPObject> IfcSpatialElementType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcSpatialElementType> copy_self( new IfcSpatialElementType() );
	if( m_GlobalId )
	{
...
void IfcSpatialElementType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSPATIALELEMENTTYPE" << "(";
...
void IfcSpatialElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSpatialElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpatialElementType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcSpatialElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcTypeProduct::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "ElementType", m_ElementType ) );
}
void IfcSpatialElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcTypeProduct::getAttributesInverse( vec_attributes_inverse );
}
void IfcSpatialElementType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcTypeProduct::setInverseCounterparts( ptr_self_entity );
}
void IfcSpatialElementType::unlinkFromInverseCounterparts()
{
