...
#include "include/IfcCooledBeamType.h"
...
IfcCooledBeamType::IfcCooledBeamType() { m_entity_enum = IFCCOOLEDBEAMTYPE; }
IfcCooledBeamType::IfcCooledBeamType( int id ) { m_id = id; m_entity_enum = IFCCOOLEDBEAMTYPE; }
IfcCooledBeamType::~IfcCooledBeamType() {}
shared_ptr<IfcPPObject> IfcCooledBeamType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcCooledBeamTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcCooledBeamType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOOLEDBEAMTYPE" << "(";
...
void IfcCooledBeamType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCooledBeamType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCooledBeamType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCooledBeamTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcCooledBeamType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcCooledBeamType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcCooledBeamType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
