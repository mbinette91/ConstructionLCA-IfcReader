...
{
	shared_ptr<IfcFooting> copy_self( new IfcFooting() );
	if( m_GlobalId )
	{
...
void IfcFooting::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCFOOTING" << "(";
...
void IfcFooting::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcFootingTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcFooting::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcFooting::unlinkFromInverseCounterparts()
{
