...
#include "include/IfcInterceptor.h"
...
shared_ptr<IfcPPObject> IfcInterceptor::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcInterceptor> copy_self( new IfcInterceptor() );
	if( m_GlobalId )
	{
...
void IfcInterceptor::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCINTERCEPTOR" << "(";
...
void IfcInterceptor::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcInterceptor::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcInterceptor, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcInterceptorTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcInterceptor::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcInterceptor::unlinkFromInverseCounterparts()
{
