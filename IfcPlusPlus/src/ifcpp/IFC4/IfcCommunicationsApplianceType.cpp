...
#include "include/IfcCommunicationsApplianceType.h"
...
{
	stream << "#" << m_id << "= IFCCOMMUNICATIONSAPPLIANCETYPE" << "(";
...
void IfcCommunicationsApplianceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCommunicationsApplianceType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCommunicationsApplianceTypeEnum::createObjectFromSTEP( args[9] );
}
