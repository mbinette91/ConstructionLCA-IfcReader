...
#include "include/IfcElectricAppliance.h"
...
shared_ptr<IfcPPObject> IfcElectricAppliance::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcElectricAppliance> copy_self( new IfcElectricAppliance() );
	if( m_GlobalId )
	{
...
void IfcElectricAppliance::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELECTRICAPPLIANCE" << "(";
...
void IfcElectricAppliance::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcElectricAppliance::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElectricAppliance, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcElectricApplianceTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcElectricAppliance::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcElectricAppliance::unlinkFromInverseCounterparts()
{
