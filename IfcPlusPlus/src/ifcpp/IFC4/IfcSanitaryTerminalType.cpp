...
#include "include/IfcSanitaryTerminalType.h"
...
shared_ptr<IfcPPObject> IfcSanitaryTerminalType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcSanitaryTerminalType> copy_self( new IfcSanitaryTerminalType() );
	if( m_GlobalId )
	{
...
void IfcSanitaryTerminalType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSANITARYTERMINALTYPE" << "(";
...
void IfcSanitaryTerminalType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSanitaryTerminalType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSanitaryTerminalType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSanitaryTerminalTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcSanitaryTerminalType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcSanitaryTerminalType::unlinkFromInverseCounterparts()
{
