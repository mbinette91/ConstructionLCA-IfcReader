...
#include "include/IfcStructuralSurfaceMember.h"
...
shared_ptr<IfcPPObject> IfcStructuralSurfaceMember::getDeepCopy( IfcPPCopyOptions& options )
{
...
{
	stream << "#" << m_id << "= IFCSTRUCTURALSURFACEMEMBER" << "(";
...
void IfcStructuralSurfaceMember::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcStructuralSurfaceMember::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralSurfaceMember, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcStructuralSurfaceMemberTypeEnum::createObjectFromSTEP( args[7] );
	m_Thickness = IfcPositiveLengthMeasure::createObjectFromSTEP( args[8] );
}
