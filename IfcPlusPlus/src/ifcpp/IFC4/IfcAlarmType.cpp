...
#include "include/IfcAlarmType.h"
...
// ENTITY IfcAlarmType 
IfcAlarmType::IfcAlarmType() { m_entity_enum = IFCALARMTYPE; }
IfcAlarmType::IfcAlarmType( int id ) { m_id = id; m_entity_enum = IFCALARMTYPE; }
IfcAlarmType::~IfcAlarmType() {}
shared_ptr<IfcPPObject> IfcAlarmType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcAlarmTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcAlarmType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCALARMTYPE" << "(";
...
void IfcAlarmType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAlarmType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAlarmTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcAlarmType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcDistributionControlElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcAlarmType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcDistributionControlElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcAlarmType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcDistributionControlElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcAlarmType::unlinkFromInverseCounterparts()
{
