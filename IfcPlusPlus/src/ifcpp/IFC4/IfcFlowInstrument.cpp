...
#include "include/IfcFlowInstrument.h"
...
IfcFlowInstrument::IfcFlowInstrument() { m_entity_enum = IFCFLOWINSTRUMENT; }
IfcFlowInstrument::IfcFlowInstrument( int id ) { m_id = id; m_entity_enum = IFCFLOWINSTRUMENT; }
IfcFlowInstrument::~IfcFlowInstrument() {}
shared_ptr<IfcPPObject> IfcFlowInstrument::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcFlowInstrument::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFlowInstrument::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowInstrument, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcFlowInstrumentTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcFlowInstrument::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
