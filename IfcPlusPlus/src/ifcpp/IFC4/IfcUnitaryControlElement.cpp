...
#include "include/IfcUnitaryControlElement.h"
...
{
	shared_ptr<IfcUnitaryControlElement> copy_self( new IfcUnitaryControlElement() );
	if( m_GlobalId )
	{
...
void IfcUnitaryControlElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCUNITARYCONTROLELEMENT" << "(";
...
void IfcUnitaryControlElement::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcUnitaryControlElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcUnitaryControlElementTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcUnitaryControlElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcUnitaryControlElement::unlinkFromInverseCounterparts()
{
