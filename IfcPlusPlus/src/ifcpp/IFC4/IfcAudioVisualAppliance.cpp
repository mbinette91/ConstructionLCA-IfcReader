...
#include "include/IfcAudioVisualAppliance.h"
...
shared_ptr<IfcPPObject> IfcAudioVisualAppliance::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcAudioVisualAppliance::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCAUDIOVISUALAPPLIANCE" << "(";
...
void IfcAudioVisualAppliance::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAudioVisualAppliance, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAudioVisualApplianceTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcAudioVisualAppliance::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcAudioVisualAppliance::unlinkFromInverseCounterparts()
{
