...
#include "include/IfcColumnStandardCase.h"
...
void IfcColumnStandardCase::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOLUMNSTANDARDCASE" << "(";
...
void IfcColumnStandardCase::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcColumnStandardCase::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcColumnStandardCase, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
