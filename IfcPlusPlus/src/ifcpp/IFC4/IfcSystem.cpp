...
// ENTITY IfcSystem 
IfcSystem::IfcSystem() { m_entity_enum = IFCSYSTEM; }
IfcSystem::IfcSystem( int id ) { m_id = id; m_entity_enum = IFCSYSTEM; }
IfcSystem::~IfcSystem() {}
shared_ptr<IfcPPObject> IfcSystem::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcSystem::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSystem, expecting 5, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcSystem::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
