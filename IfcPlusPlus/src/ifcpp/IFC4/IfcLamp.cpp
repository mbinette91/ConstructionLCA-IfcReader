...
IfcLamp::~IfcLamp() {}
shared_ptr<IfcPPObject> IfcLamp::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcLamp::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCLAMP" << "(";
...
void IfcLamp::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcLamp, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcLampTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcLamp::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcLamp::unlinkFromInverseCounterparts()
{
