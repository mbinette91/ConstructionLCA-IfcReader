...
#include "include/IfcPipeFitting.h"
...
IfcPipeFitting::IfcPipeFitting() { m_entity_enum = IFCPIPEFITTING; }
IfcPipeFitting::IfcPipeFitting( int id ) { m_id = id; m_entity_enum = IFCPIPEFITTING; }
...
void IfcPipeFitting::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPipeFitting::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPipeFitting, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcPipeFittingTypeEnum::createObjectFromSTEP( args[8] );
}
