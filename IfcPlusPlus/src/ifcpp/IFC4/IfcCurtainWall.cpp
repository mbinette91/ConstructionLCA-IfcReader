...
#include "include/IfcCurtainWall.h"
...
shared_ptr<IfcPPObject> IfcCurtainWall::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCurtainWall> copy_self( new IfcCurtainWall() );
	if( m_GlobalId )
	{
...
void IfcCurtainWall::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCURTAINWALL" << "(";
...
void IfcCurtainWall::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCurtainWall::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCurtainWall, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCurtainWallTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCurtainWall::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCurtainWall::unlinkFromInverseCounterparts()
{
