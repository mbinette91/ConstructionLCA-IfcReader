...
#include "include/IfcLightFixture.h"
...
shared_ptr<IfcPPObject> IfcLightFixture::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcLightFixture> copy_self( new IfcLightFixture() );
	if( m_GlobalId )
	{
...
void IfcLightFixture::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCLIGHTFIXTURE" << "(";
...
void IfcLightFixture::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcLightFixture::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcLightFixture, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcLightFixtureTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcLightFixture::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcLightFixture::unlinkFromInverseCounterparts()
{
