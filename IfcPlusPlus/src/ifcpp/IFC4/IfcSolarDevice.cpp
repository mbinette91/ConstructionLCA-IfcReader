...
#include "include/IfcSolarDevice.h"
#include "include/IfcSolarDeviceTypeEnum.h"
#include "include/IfcText.h"

// ENTITY IfcSolarDevice 
IfcSolarDevice::IfcSolarDevice() { m_entity_enum = IFCSOLARDEVICE; }
IfcSolarDevice::IfcSolarDevice( int id ) { m_id = id; m_entity_enum = IFCSOLARDEVICE; }
IfcSolarDevice::~IfcSolarDevice() {}
shared_ptr<IfcPPObject> IfcSolarDevice::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcSolarDevice::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSOLARDEVICE" << "(";
...
void IfcSolarDevice::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSolarDevice::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSolarDevice, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSolarDeviceTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcSolarDevice::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
