...
IfcOpeningElement::IfcOpeningElement() { m_entity_enum = IFCOPENINGELEMENT; }
IfcOpeningElement::IfcOpeningElement( int id ) { m_id = id; m_entity_enum = IFCOPENINGELEMENT; }
...
void IfcOpeningElement::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcOpeningElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcOpeningElement, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcOpeningElement::unlinkFromInverseCounterparts()
{
