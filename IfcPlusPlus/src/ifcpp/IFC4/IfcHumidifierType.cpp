...
#include "include/IfcHumidifierType.h"
...
shared_ptr<IfcPPObject> IfcHumidifierType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcHumidifierType> copy_self( new IfcHumidifierType() );
	if( m_GlobalId )
	{
...
void IfcHumidifierType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCHUMIDIFIERTYPE" << "(";
...
void IfcHumidifierType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcHumidifierType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcHumidifierType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcHumidifierTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcHumidifierType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcHumidifierType::unlinkFromInverseCounterparts()
{
