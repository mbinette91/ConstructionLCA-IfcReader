...
#include "include/IfcVibrationIsolatorType.h"
...
shared_ptr<IfcPPObject> IfcVibrationIsolatorType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcVibrationIsolatorTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcVibrationIsolatorType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCVIBRATIONISOLATORTYPE" << "(";
...
void IfcVibrationIsolatorType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcVibrationIsolatorType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcVibrationIsolatorType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcVibrationIsolatorTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcVibrationIsolatorType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcElementComponentType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcVibrationIsolatorType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcElementComponentType::getAttributesInverse( vec_attributes_inverse );
}
void IfcVibrationIsolatorType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcElementComponentType::setInverseCounterparts( ptr_self_entity );
}
void IfcVibrationIsolatorType::unlinkFromInverseCounterparts()
{
