...
IfcTank::~IfcTank() {}
shared_ptr<IfcPPObject> IfcTank::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcTank::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCTANK" << "(";
...
void IfcTank::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcTank::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTank, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcTankTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcTank::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcTank::unlinkFromInverseCounterparts()
{
