...
void IfcEventType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcEventTypeEnum::createObjectFromSTEP( args[9] );
	m_EventTriggerType = IfcEventTriggerTypeEnum::createObjectFromSTEP( args[10] );
	m_UserDefinedEventTriggerType = IfcLabel::createObjectFromSTEP( args[11] );
}
void IfcEventType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
