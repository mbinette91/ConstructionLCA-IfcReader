...
#include "include/IfcBuildingElementPartType.h"
...
shared_ptr<IfcPPObject> IfcBuildingElementPartType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcBuildingElementPartType> copy_self( new IfcBuildingElementPartType() );
	if( m_GlobalId )
	{
...
void IfcBuildingElementPartType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCBUILDINGELEMENTPARTTYPE" << "(";
...
void IfcBuildingElementPartType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcBuildingElementPartType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBuildingElementPartType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBuildingElementPartTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcBuildingElementPartType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcBuildingElementPartType::unlinkFromInverseCounterparts()
{
