...
#include "include/IfcHumidifier.h"
...
IfcHumidifier::IfcHumidifier() { m_entity_enum = IFCHUMIDIFIER; }
IfcHumidifier::IfcHumidifier( int id ) { m_id = id; m_entity_enum = IFCHUMIDIFIER; }
IfcHumidifier::~IfcHumidifier() {}
shared_ptr<IfcPPObject> IfcHumidifier::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcHumidifierTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcHumidifier::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCHUMIDIFIER" << "(";
...
void IfcHumidifier::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcHumidifier, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcHumidifierTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcHumidifier::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDevice::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcHumidifier::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDevice::getAttributesInverse( vec_attributes_inverse );
}
void IfcHumidifier::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
