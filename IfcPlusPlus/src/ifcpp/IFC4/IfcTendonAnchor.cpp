...
#include "include/IfcTendonAnchor.h"
...
shared_ptr<IfcPPObject> IfcTendonAnchor::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcTendonAnchor::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCTENDONANCHOR" << "(";
...
void IfcTendonAnchor::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcTendonAnchor::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTendonAnchor, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcTendonAnchor::unlinkFromInverseCounterparts()
{
