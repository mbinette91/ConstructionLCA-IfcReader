...
shared_ptr<IfcPPObject> IfcCableSegmentType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCableSegmentType> copy_self( new IfcCableSegmentType() );
	if( m_GlobalId )
	{
...
void IfcCableSegmentType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCableSegmentType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCableSegmentTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcCableSegmentType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCableSegmentType::unlinkFromInverseCounterparts()
{
