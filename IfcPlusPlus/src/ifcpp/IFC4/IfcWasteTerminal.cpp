...
#include "include/IfcWasteTerminal.h"
...
void IfcWasteTerminal::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCWASTETERMINAL" << "(";
...
void IfcWasteTerminal::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcWasteTerminal::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWasteTerminal, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcWasteTerminalTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcWasteTerminal::unlinkFromInverseCounterparts()
{
