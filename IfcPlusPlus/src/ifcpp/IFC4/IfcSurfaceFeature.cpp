...
#include "include/IfcSurfaceFeature.h"
...
shared_ptr<IfcPPObject> IfcSurfaceFeature::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcSurfaceFeatureTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcSurfaceFeature::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSURFACEFEATURE" << "(";
...
void IfcSurfaceFeature::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSurfaceFeature::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcSurfaceFeatureTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcSurfaceFeature::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFeatureElement::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcSurfaceFeature::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFeatureElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcSurfaceFeature::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
