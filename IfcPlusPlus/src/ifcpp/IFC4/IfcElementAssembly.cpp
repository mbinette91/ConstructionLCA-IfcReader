...
#include "include/IfcElementAssembly.h"
...
shared_ptr<IfcPPObject> IfcElementAssembly::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcElementAssembly> copy_self( new IfcElementAssembly() );
	if( m_GlobalId )
	{
	...
	if( m_AssemblyPlace ) { copy_self->m_AssemblyPlace = dynamic_pointer_cast<IfcAssemblyPlaceEnum>( m_AssemblyPlace->getDeepCopy(options) ); }
...
void IfcElementAssembly::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELEMENTASSEMBLY" << "(";
...
void IfcElementAssembly::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcElementAssembly::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementAssembly, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_AssemblyPlace = IfcAssemblyPlaceEnum::createObjectFromSTEP( args[8] );
...
void IfcElementAssembly::unlinkFromInverseCounterparts()
{
