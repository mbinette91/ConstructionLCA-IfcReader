...
#include "include/IfcCableCarrierSegment.h"
...
shared_ptr<IfcPPObject> IfcCableCarrierSegment::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCableCarrierSegment> copy_self( new IfcCableCarrierSegment() );
	if( m_GlobalId )
	{
...
void IfcCableCarrierSegment::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCableCarrierSegment, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCableCarrierSegmentTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCableCarrierSegment::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCableCarrierSegment::unlinkFromInverseCounterparts()
{
