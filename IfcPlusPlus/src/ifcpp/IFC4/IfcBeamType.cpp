...
shared_ptr<IfcPPObject> IfcBeamType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcBeamType> copy_self( new IfcBeamType() );
	if( m_GlobalId )
	{
...
void IfcBeamType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBeamType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBeamTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcBeamType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcBeamType::unlinkFromInverseCounterparts()
{
