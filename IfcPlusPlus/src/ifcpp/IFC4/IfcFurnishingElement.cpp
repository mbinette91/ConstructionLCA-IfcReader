...
shared_ptr<IfcPPObject> IfcFurnishingElement::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcFurnishingElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFurnishingElement, expecting 8, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcFurnishingElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcFurnishingElement::unlinkFromInverseCounterparts()
{
