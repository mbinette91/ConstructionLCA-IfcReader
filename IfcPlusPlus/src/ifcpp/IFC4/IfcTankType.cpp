...
#include "include/IfcTankType.h"
#include "include/IfcTankTypeEnum.h"
#include "include/IfcText.h"

// ENTITY IfcTankType 
IfcTankType::IfcTankType() { m_entity_enum = IFCTANKTYPE; }
IfcTankType::IfcTankType( int id ) { m_id = id; m_entity_enum = IFCTANKTYPE; }
...
void IfcTankType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTankType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcTankTypeEnum::createObjectFromSTEP( args[9] );
}
