...
#include "include/IfcChimneyType.h"
...
IfcChimneyType::IfcChimneyType() { m_entity_enum = IFCCHIMNEYTYPE; }
IfcChimneyType::IfcChimneyType( int id ) { m_id = id; m_entity_enum = IFCCHIMNEYTYPE; }
IfcChimneyType::~IfcChimneyType() {}
shared_ptr<IfcPPObject> IfcChimneyType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcChimneyTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcChimneyType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCHIMNEYTYPE" << "(";
...
void IfcChimneyType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcChimneyTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcChimneyType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcBuildingElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcChimneyType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcBuildingElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcChimneyType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcBuildingElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcChimneyType::unlinkFromInverseCounterparts()
{
