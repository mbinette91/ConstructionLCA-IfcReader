...
shared_ptr<IfcPPObject> IfcExternalSpatialElement::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcExternalSpatialElementTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
...
void IfcExternalSpatialElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcExternalSpatialElementTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcExternalSpatialElement::unlinkFromInverseCounterparts()
{
