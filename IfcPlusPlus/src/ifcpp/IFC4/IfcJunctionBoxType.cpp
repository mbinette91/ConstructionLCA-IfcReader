...
#include "include/IfcJunctionBoxType.h"
...
shared_ptr<IfcPPObject> IfcJunctionBoxType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcJunctionBoxType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCJUNCTIONBOXTYPE" << "(";
...
void IfcJunctionBoxType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcJunctionBoxType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcJunctionBoxType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcJunctionBoxTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcJunctionBoxType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcJunctionBoxType::unlinkFromInverseCounterparts()
{
