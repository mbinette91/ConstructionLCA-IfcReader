...
#include "include/IfcShadingDeviceType.h"
#include "include/IfcShadingDeviceTypeEnum.h"
#include "include/IfcText.h"

// ENTITY IfcShadingDeviceType 
IfcShadingDeviceType::IfcShadingDeviceType() { m_entity_enum = IFCSHADINGDEVICETYPE; }
IfcShadingDeviceType::IfcShadingDeviceType( int id ) { m_id = id; m_entity_enum = IFCSHADINGDEVICETYPE; }
IfcShadingDeviceType::~IfcShadingDeviceType() {}
shared_ptr<IfcPPObject> IfcShadingDeviceType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcShadingDeviceType> copy_self( new IfcShadingDeviceType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcShadingDeviceTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcShadingDeviceType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSHADINGDEVICETYPE" << "(";
...
void IfcShadingDeviceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcShadingDeviceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcShadingDeviceType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcShadingDeviceTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcShadingDeviceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
