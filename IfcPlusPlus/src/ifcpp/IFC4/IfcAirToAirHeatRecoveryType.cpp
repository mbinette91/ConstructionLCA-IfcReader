...
#include "include/IfcAirToAirHeatRecoveryType.h"
...
shared_ptr<IfcPPObject> IfcAirToAirHeatRecoveryType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcAirToAirHeatRecoveryType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCAIRTOAIRHEATRECOVERYTYPE" << "(";
...
void IfcAirToAirHeatRecoveryType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcAirToAirHeatRecoveryType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAirToAirHeatRecoveryType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAirToAirHeatRecoveryTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcAirToAirHeatRecoveryType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcAirToAirHeatRecoveryType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcAirToAirHeatRecoveryType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
