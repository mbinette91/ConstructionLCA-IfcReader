...
#include "include/IfcEvaporativeCoolerType.h"
...
shared_ptr<IfcPPObject> IfcEvaporativeCoolerType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcEvaporativeCoolerType> copy_self( new IfcEvaporativeCoolerType() );
	if( m_GlobalId )
	{
...
void IfcEvaporativeCoolerType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCEVAPORATIVECOOLERTYPE" << "(";
...
void IfcEvaporativeCoolerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcEvaporativeCoolerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEvaporativeCoolerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcEvaporativeCoolerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcEvaporativeCoolerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcEvaporativeCoolerType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcEvaporativeCoolerType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcEnergyConversionDeviceType::setInverseCounterparts( ptr_self_entity );
}
void IfcEvaporativeCoolerType::unlinkFromInverseCounterparts()
{
