...
#include "include/IfcCoveringType.h"
...
shared_ptr<IfcPPObject> IfcCoveringType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcCoveringTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcCoveringType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOVERINGTYPE" << "(";
...
void IfcCoveringType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCoveringType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCoveringType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCoveringTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcCoveringType::unlinkFromInverseCounterparts()
{
