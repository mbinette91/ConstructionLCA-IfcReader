...
IfcValve::~IfcValve() {}
shared_ptr<IfcPPObject> IfcValve::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcValve> copy_self( new IfcValve() );
	if( m_GlobalId )
	{
...
void IfcValve::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCVALVE" << "(";
...
void IfcValve::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcValve::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcValve, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcValveTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcValve::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcValve::unlinkFromInverseCounterparts()
{
