...
#include "include/IfcAirTerminalType.h"
...
shared_ptr<IfcPPObject> IfcAirTerminalType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcAirTerminalType> copy_self( new IfcAirTerminalType() );
	if( m_GlobalId )
	{
...
void IfcAirTerminalType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCAIRTERMINALTYPE" << "(";
...
void IfcAirTerminalType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcAirTerminalType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAirTerminalType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAirTerminalTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcAirTerminalType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcAirTerminalType::unlinkFromInverseCounterparts()
{
