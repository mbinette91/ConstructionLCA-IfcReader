...
shared_ptr<IfcPPObject> IfcDuctSilencerType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcDuctSilencerTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
...
void IfcDuctSilencerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDuctSilencerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDuctSilencerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcDuctSilencerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowTreatmentDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcDuctSilencerType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowTreatmentDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcDuctSilencerType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
