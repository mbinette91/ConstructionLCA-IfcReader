...
#include "include/IfcElectricFlowStorageDevice.h"
...
shared_ptr<IfcPPObject> IfcElectricFlowStorageDevice::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcElectricFlowStorageDevice> copy_self( new IfcElectricFlowStorageDevice() );
	if( m_GlobalId )
	{
...
void IfcElectricFlowStorageDevice::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELECTRICFLOWSTORAGEDEVICE" << "(";
...
void IfcElectricFlowStorageDevice::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElectricFlowStorageDevice, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcElectricFlowStorageDeviceTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcElectricFlowStorageDevice::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
