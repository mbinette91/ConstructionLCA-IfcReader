...
#include "include/IfcPlateType.h"
...
void IfcPlateType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPLATETYPE" << "(";
...
void IfcPlateType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPlateType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcPlateTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcPlateType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcPlateType::unlinkFromInverseCounterparts()
{
