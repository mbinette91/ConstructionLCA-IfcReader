...
#include "include/IfcMemberType.h"
...
IfcMemberType::IfcMemberType() { m_entity_enum = IFCMEMBERTYPE; }
IfcMemberType::IfcMemberType( int id ) { m_id = id; m_entity_enum = IFCMEMBERTYPE; }
IfcMemberType::~IfcMemberType() {}
shared_ptr<IfcPPObject> IfcMemberType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcMemberType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCMEMBERTYPE" << "(";
...
void IfcMemberType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcMemberType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMemberType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcMemberTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcMemberType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcBuildingElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcMemberType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcBuildingElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcMemberType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcBuildingElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcMemberType::unlinkFromInverseCounterparts()
{
