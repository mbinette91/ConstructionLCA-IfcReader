...
#include "include/IfcCooledBeam.h"
...
IfcCooledBeam::IfcCooledBeam() { m_entity_enum = IFCCOOLEDBEAM; }
IfcCooledBeam::IfcCooledBeam( int id ) { m_id = id; m_entity_enum = IFCCOOLEDBEAM; }
IfcCooledBeam::~IfcCooledBeam() {}
shared_ptr<IfcPPObject> IfcCooledBeam::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcCooledBeam> copy_self( new IfcCooledBeam() );
	if( m_GlobalId )
	{
...
void IfcCooledBeam::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOOLEDBEAM" << "(";
...
void IfcCooledBeam::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCooledBeam::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCooledBeam, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCooledBeamTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCooledBeam::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcCooledBeam::unlinkFromInverseCounterparts()
{
