...
#include "include/IfcVibrationIsolator.h"
...
shared_ptr<IfcPPObject> IfcVibrationIsolator::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcVibrationIsolator> copy_self( new IfcVibrationIsolator() );
	if( m_GlobalId )
	{
...
void IfcVibrationIsolator::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCVIBRATIONISOLATOR" << "(";
...
void IfcVibrationIsolator::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcVibrationIsolator::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcVibrationIsolator, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcVibrationIsolatorTypeEnum::createObjectFromSTEP( args[8] );
}
