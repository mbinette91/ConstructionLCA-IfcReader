...
shared_ptr<IfcPPObject> IfcCivilElementType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcCivilElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCivilElementType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcCivilElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcElementType::getAttributes( vec_attributes );
}
void IfcCivilElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcCivilElementType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcCivilElementType::unlinkFromInverseCounterparts()
{
