...
IfcCoil::~IfcCoil() {}
shared_ptr<IfcPPObject> IfcCoil::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcCoil::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOIL" << "(";
...
void IfcCoil::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCoil, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCoilTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCoil::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcEnergyConversionDevice::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcCoil::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcEnergyConversionDevice::getAttributesInverse( vec_attributes_inverse );
}
void IfcCoil::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcEnergyConversionDevice::setInverseCounterparts( ptr_self_entity );
}
void IfcCoil::unlinkFromInverseCounterparts()
{
