...
#include "include/IfcProtectiveDeviceTrippingUnit.h"
...
IfcProtectiveDeviceTrippingUnit::IfcProtectiveDeviceTrippingUnit() { m_entity_enum = IFCPROTECTIVEDEVICETRIPPINGUNIT; }
IfcProtectiveDeviceTrippingUnit::IfcProtectiveDeviceTrippingUnit( int id ) { m_id = id; m_entity_enum = IFCPROTECTIVEDEVICETRIPPINGUNIT; }
IfcProtectiveDeviceTrippingUnit::~IfcProtectiveDeviceTrippingUnit() {}
shared_ptr<IfcPPObject> IfcProtectiveDeviceTrippingUnit::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcProtectiveDeviceTrippingUnit> copy_self( new IfcProtectiveDeviceTrippingUnit() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcProtectiveDeviceTrippingUnitTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcProtectiveDeviceTrippingUnit::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPROTECTIVEDEVICETRIPPINGUNIT" << "(";
...
void IfcProtectiveDeviceTrippingUnit::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcProtectiveDeviceTrippingUnit, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcProtectiveDeviceTrippingUnitTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcProtectiveDeviceTrippingUnit::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcProtectiveDeviceTrippingUnit::unlinkFromInverseCounterparts()
{
