...
shared_ptr<IfcPPObject> IfcPumpType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcPumpType> copy_self( new IfcPumpType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcPumpTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcPumpType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPUMPTYPE" << "(";
...
void IfcPumpType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPumpType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPumpType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcPumpTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcPumpType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowMovingDeviceType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcPumpType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowMovingDeviceType::getAttributesInverse( vec_attributes_inverse );
}
void IfcPumpType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
