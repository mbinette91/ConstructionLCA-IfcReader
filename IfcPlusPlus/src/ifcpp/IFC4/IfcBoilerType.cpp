...
#include "include/IfcBoilerType.h"
...
shared_ptr<IfcPPObject> IfcBoilerType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcBoilerType> copy_self( new IfcBoilerType() );
	if( m_GlobalId )
	{
...
void IfcBoilerType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCBOILERTYPE" << "(";
...
void IfcBoilerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcBoilerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBoilerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcBoilerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcBoilerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcBoilerType::unlinkFromInverseCounterparts()
{
