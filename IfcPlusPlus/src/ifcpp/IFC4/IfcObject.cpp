...
// ENTITY IfcObject 
IfcObject::IfcObject() { m_entity_enum = IFCOBJECT; }
IfcObject::IfcObject( int id ) { m_id = id; m_entity_enum = IFCOBJECT; }
IfcObject::~IfcObject() {}
...
void IfcObject::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcObject, expecting 5, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
