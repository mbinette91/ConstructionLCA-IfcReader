...
// ENTITY IfcLampType 
IfcLampType::IfcLampType() { m_entity_enum = IFCLAMPTYPE; }
IfcLampType::IfcLampType( int id ) { m_id = id; m_entity_enum = IFCLAMPTYPE; }
IfcLampType::~IfcLampType() {}
shared_ptr<IfcPPObject> IfcLampType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcLampType> copy_self( new IfcLampType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcLampTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcLampType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCLAMPTYPE" << "(";
...
void IfcLampType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcLampType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcLampTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcLampType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcLampType::unlinkFromInverseCounterparts()
{
