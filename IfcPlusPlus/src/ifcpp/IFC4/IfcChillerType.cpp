...
#include "include/IfcChillerType.h"
...
IfcChillerType::IfcChillerType() { m_entity_enum = IFCCHILLERTYPE; }
IfcChillerType::IfcChillerType( int id ) { m_id = id; m_entity_enum = IFCCHILLERTYPE; }
IfcChillerType::~IfcChillerType() {}
shared_ptr<IfcPPObject> IfcChillerType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcChillerType> copy_self( new IfcChillerType() );
	if( m_GlobalId )
	{
...
void IfcChillerType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCHILLERTYPE" << "(";
...
void IfcChillerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcChillerType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcChillerType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcChillerTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcChillerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcChillerType::unlinkFromInverseCounterparts()
{
