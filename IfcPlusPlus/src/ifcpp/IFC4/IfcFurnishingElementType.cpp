...
#include "include/IfcFurnishingElementType.h"
...
shared_ptr<IfcPPObject> IfcFurnishingElementType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcFurnishingElementType> copy_self( new IfcFurnishingElementType() );
	if( m_GlobalId )
	{
...
void IfcFurnishingElementType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCFURNISHINGELEMENTTYPE" << "(";
...
void IfcFurnishingElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFurnishingElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFurnishingElementType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcFurnishingElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcFurnishingElementType::unlinkFromInverseCounterparts()
{
