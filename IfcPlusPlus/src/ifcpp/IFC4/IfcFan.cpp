...
IfcFan::~IfcFan() {}
shared_ptr<IfcPPObject> IfcFan::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcFan> copy_self( new IfcFan() );
	if( m_GlobalId )
	{
...
void IfcFan::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFan, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcFanTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcFan::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowMovingDevice::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcFan::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowMovingDevice::getAttributesInverse( vec_attributes_inverse );
}
void IfcFan::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcFlowMovingDevice::setInverseCounterparts( ptr_self_entity );
}
void IfcFan::unlinkFromInverseCounterparts()
{
