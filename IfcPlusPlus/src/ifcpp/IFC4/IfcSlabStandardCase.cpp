...
#include "include/IfcSlabStandardCase.h"
...
{
	shared_ptr<IfcSlabStandardCase> copy_self( new IfcSlabStandardCase() );
	if( m_GlobalId )
	{
...
void IfcSlabStandardCase::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
...
void IfcSlabStandardCase::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcSlabStandardCase::unlinkFromInverseCounterparts()
{
