...
#include "include/IfcSwitchingDeviceType.h"
#include "include/IfcSwitchingDeviceTypeEnum.h"
#include "include/IfcText.h"

// ENTITY IfcSwitchingDeviceType 
IfcSwitchingDeviceType::IfcSwitchingDeviceType() { m_entity_enum = IFCSWITCHINGDEVICETYPE; }
IfcSwitchingDeviceType::IfcSwitchingDeviceType( int id ) { m_id = id; m_entity_enum = IFCSWITCHINGDEVICETYPE; }
IfcSwitchingDeviceType::~IfcSwitchingDeviceType() {}
shared_ptr<IfcPPObject> IfcSwitchingDeviceType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcSwitchingDeviceType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSWITCHINGDEVICETYPE" << "(";
...
void IfcSwitchingDeviceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcSwitchingDeviceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSwitchingDeviceType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSwitchingDeviceTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcSwitchingDeviceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
