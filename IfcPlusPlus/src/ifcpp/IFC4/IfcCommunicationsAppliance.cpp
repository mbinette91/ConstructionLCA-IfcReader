...
#include "include/IfcCommunicationsAppliance.h"
...
IfcCommunicationsAppliance::IfcCommunicationsAppliance() { m_entity_enum = IFCCOMMUNICATIONSAPPLIANCE; }
IfcCommunicationsAppliance::IfcCommunicationsAppliance( int id ) { m_id = id; m_entity_enum = IFCCOMMUNICATIONSAPPLIANCE; }
IfcCommunicationsAppliance::~IfcCommunicationsAppliance() {}
shared_ptr<IfcPPObject> IfcCommunicationsAppliance::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcCommunicationsAppliance::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCCOMMUNICATIONSAPPLIANCE" << "(";
...
void IfcCommunicationsAppliance::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcCommunicationsAppliance::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCommunicationsAppliance, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCommunicationsApplianceTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcCommunicationsAppliance::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
