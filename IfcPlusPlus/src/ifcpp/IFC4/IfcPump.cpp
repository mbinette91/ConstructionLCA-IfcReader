...
IfcPump::~IfcPump() {}
shared_ptr<IfcPPObject> IfcPump::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcPump::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPUMP" << "(";
...
void IfcPump::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPump::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPump, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcPumpTypeEnum::createObjectFromSTEP( args[8] );
}
...
void IfcPump::unlinkFromInverseCounterparts()
{
