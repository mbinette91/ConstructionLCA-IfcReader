...
#include "include/IfcElementComponent.h"
...
shared_ptr<IfcPPObject> IfcElementComponent::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcElementComponent::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELEMENTCOMPONENT" << "(";
...
void IfcElementComponent::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementComponent, expecting 8, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcElementComponent::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcElement::getAttributes( vec_attributes );
}
void IfcElementComponent::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcElementComponent::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcElement::setInverseCounterparts( ptr_self_entity );
}
void IfcElementComponent::unlinkFromInverseCounterparts()
{
