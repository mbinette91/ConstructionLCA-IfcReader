...
#include "include/IfcDuctFitting.h"
...
shared_ptr<IfcPPObject> IfcDuctFitting::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDuctFitting> copy_self( new IfcDuctFitting() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcDuctFittingTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcDuctFitting::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDUCTFITTING" << "(";
...
void IfcDuctFitting::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcDuctFitting::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDuctFitting, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDuctFittingTypeEnum::createObjectFromSTEP( args[8] );
}
void IfcDuctFitting::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcDuctFitting::unlinkFromInverseCounterparts()
{
