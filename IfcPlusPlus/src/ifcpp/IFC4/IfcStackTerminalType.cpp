...
#include "include/IfcStackTerminalType.h"
...
shared_ptr<IfcPPObject> IfcStackTerminalType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcStackTerminalType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCSTACKTERMINALTYPE" << "(";
...
void IfcStackTerminalType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcStackTerminalType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStackTerminalType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcStackTerminalTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcStackTerminalType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcStackTerminalType::unlinkFromInverseCounterparts()
{
