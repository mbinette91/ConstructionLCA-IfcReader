...
#include "include/IfcRailingType.h"
...
shared_ptr<IfcPPObject> IfcRailingType::getDeepCopy( IfcPPCopyOptions& options )
{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcRailingTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcRailingType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCRAILINGTYPE" << "(";
...
void IfcRailingType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcRailingType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRailingType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcRailingTypeEnum::createObjectFromSTEP( args[9] );
}
...
void IfcRailingType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
...
void IfcRailingType::unlinkFromInverseCounterparts()
{
