...
#include "include/IfcMedicalDeviceType.h"
...
shared_ptr<IfcPPObject> IfcMedicalDeviceType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcMedicalDeviceType> copy_self( new IfcMedicalDeviceType() );
	if( m_GlobalId )
	{
...
void IfcMedicalDeviceType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCMEDICALDEVICETYPE" << "(";
...
void IfcMedicalDeviceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcMedicalDeviceType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMedicalDeviceType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcMedicalDeviceTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcMedicalDeviceType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
