...
// ENTITY IfcPlate 
IfcPlate::IfcPlate() { m_entity_enum = IFCPLATE; }
IfcPlate::IfcPlate( int id ) { m_id = id; m_entity_enum = IFCPLATE; }
IfcPlate::~IfcPlate() {}
shared_ptr<IfcPPObject> IfcPlate::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcPlate> copy_self( new IfcPlate() );
	if( m_GlobalId )
	{
...
void IfcPlate::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCPLATE" << "(";
...
void IfcPlate::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcPlate::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPlate, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcPlate::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcPlate::unlinkFromInverseCounterparts()
{
