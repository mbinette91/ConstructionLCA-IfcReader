...
IfcDistributionFlowElement::IfcDistributionFlowElement() { m_entity_enum = IFCDISTRIBUTIONFLOWELEMENT; }
IfcDistributionFlowElement::IfcDistributionFlowElement( int id ) { m_id = id; m_entity_enum = IFCDISTRIBUTIONFLOWELEMENT; }
...
void IfcDistributionFlowElement::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDistributionFlowElement, expecting 8, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcDistributionFlowElement::unlinkFromInverseCounterparts()
{
