...
IfcRelConnects::IfcRelConnects() { m_entity_enum = IFCRELCONNECTS; }
IfcRelConnects::IfcRelConnects( int id ) { m_id = id; m_entity_enum = IFCRELCONNECTS; }
...
void IfcRelConnects::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelConnects, expecting 4, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
