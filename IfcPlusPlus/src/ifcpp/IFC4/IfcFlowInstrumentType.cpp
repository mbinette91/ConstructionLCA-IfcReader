...
#include "include/IfcFlowInstrumentType.h"
...
IfcFlowInstrumentType::IfcFlowInstrumentType() { m_entity_enum = IFCFLOWINSTRUMENTTYPE; }
IfcFlowInstrumentType::IfcFlowInstrumentType( int id ) { m_id = id; m_entity_enum = IFCFLOWINSTRUMENTTYPE; }
IfcFlowInstrumentType::~IfcFlowInstrumentType() {}
shared_ptr<IfcPPObject> IfcFlowInstrumentType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcFlowInstrumentType> copy_self( new IfcFlowInstrumentType() );
	if( m_GlobalId )
	{
...
void IfcFlowInstrumentType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCFLOWINSTRUMENTTYPE" << "(";
...
void IfcFlowInstrumentType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFlowInstrumentType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowInstrumentType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcFlowInstrumentTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcFlowInstrumentType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcDistributionControlElementType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcFlowInstrumentType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcDistributionControlElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcFlowInstrumentType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
	IfcDistributionControlElementType::setInverseCounterparts( ptr_self_entity );
}
void IfcFlowInstrumentType::unlinkFromInverseCounterparts()
{
