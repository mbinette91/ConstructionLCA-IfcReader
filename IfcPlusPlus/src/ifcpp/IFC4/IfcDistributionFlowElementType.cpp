...
#include "include/IfcDistributionFlowElementType.h"
...
shared_ptr<IfcPPObject> IfcDistributionFlowElementType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDistributionFlowElementType> copy_self( new IfcDistributionFlowElementType() );
	if( m_GlobalId )
	{
...
void IfcDistributionFlowElementType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDISTRIBUTIONFLOWELEMENTTYPE" << "(";
...
void IfcDistributionFlowElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcDistributionFlowElementType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDistributionFlowElementType, expecting 9, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
...
void IfcDistributionFlowElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcDistributionElementType::getAttributesInverse( vec_attributes_inverse );
}
...
void IfcDistributionFlowElementType::unlinkFromInverseCounterparts()
{
