...
#include "include/IfcElectricTimeControlType.h"
...
shared_ptr<IfcPPObject> IfcElectricTimeControlType::getDeepCopy( IfcPPCopyOptions& options )
{
...
void IfcElectricTimeControlType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCELECTRICTIMECONTROLTYPE" << "(";
...
void IfcElectricTimeControlType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcElectricTimeControlType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElectricTimeControlType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcElectricTimeControlTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcElectricTimeControlType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
	IfcFlowControllerType::getAttributes( vec_attributes );
	vec_attributes.push_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcElectricTimeControlType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes_inverse )
{
	IfcFlowControllerType::getAttributesInverse( vec_attributes_inverse );
}
void IfcElectricTimeControlType::setInverseCounterparts( shared_ptr<IfcPPEntity> ptr_self_entity )
{
