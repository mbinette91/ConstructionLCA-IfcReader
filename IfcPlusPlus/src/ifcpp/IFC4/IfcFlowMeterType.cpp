...
#include "include/IfcFlowMeterType.h"
...
shared_ptr<IfcPPObject> IfcFlowMeterType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcFlowMeterType> copy_self( new IfcFlowMeterType() );
	if( m_GlobalId )
	{
	...
	if( m_PredefinedType ) { copy_self->m_PredefinedType = dynamic_pointer_cast<IfcFlowMeterTypeEnum>( m_PredefinedType->getDeepCopy(options) ); }
	return copy_self;
}
void IfcFlowMeterType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCFLOWMETERTYPE" << "(";
...
void IfcFlowMeterType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcFlowMeterType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFlowMeterType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcFlowMeterTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcFlowMeterType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
...
void IfcFlowMeterType::unlinkFromInverseCounterparts()
{
