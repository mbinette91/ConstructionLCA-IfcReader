...
#include "include/IfcDamperType.h"
...
IfcDamperType::IfcDamperType() { m_entity_enum = IFCDAMPERTYPE; }
IfcDamperType::IfcDamperType( int id ) { m_id = id; m_entity_enum = IFCDAMPERTYPE; }
IfcDamperType::~IfcDamperType() {}
shared_ptr<IfcPPObject> IfcDamperType::getDeepCopy( IfcPPCopyOptions& options )
{
	shared_ptr<IfcDamperType> copy_self( new IfcDamperType() );
	if( m_GlobalId )
	{
...
void IfcDamperType::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_id << "= IFCDAMPERTYPE" << "(";
...
void IfcDamperType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_id; }
void IfcDamperType::readStepArguments( const std::vector<std::wstring>& args, const boost::unordered_map<int,shared_ptr<IfcPPEntity> >& map )
{
	const int num_args = (int)args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDamperType, expecting 10, having " << num_args << ". Entity ID: " << m_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDamperTypeEnum::createObjectFromSTEP( args[9] );
}
void IfcDamperType::getAttributes( std::vector<std::pair<std::string, shared_ptr<IfcPPObject> > >& vec_attributes )
{
